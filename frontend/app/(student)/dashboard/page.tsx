    ...
    >
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3 pb-4 border-b border-slate-100">
          ...
          {/* Badge row — flex-wrap ensures badges stack on 320px */}
          <div className="flex flex-wrap items-center gap-2">
            <div className="rounded-full bg-indigo-50 border border-indigo-100/50 px-3 py-1 text-xs font-bold text-indigo-600 flex items-center gap-1">
              <Sparkles className="h-3 w-3 text-indigo-400" />
              <span>Up to 25 MB</span>
            </div>
            <div className="rounded-full bg-slate-50 border border-slate-200 px-3 py-1 text-xs font-bold text-slate-600 flex items-center gap-1">
              <GraduationCap className="h-3 w-3 text-slate-400" />
              <span>{profileLoading ? "Loading..." : course || "All Courses"}</span>
            </div>
          </div>
        </div>
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="relative mt-2">
                    ...
                    <input
                      ...
                      className="w-full rounded-2xl border border-slate-200 bg-slate-50/50 py-3.5 pl-11 pr-28 text-sm font-medium text-slate-500 transition focus:bg-white file:hidden"
                    />
                    {/* Size indicator — absolute but clipped by pr-28 above */}
                    <div className="absolute inset-y-0 right-3 flex items-center pointer-events-none">
                      <span className="rounded-lg bg-slate-100 px-2 py-1 text-[10px] font-bold text-slate-500 whitespace-nowrap">
                        {upload ? `${(upload.size / 1024 / 1024).toFixed(1)} MB` : "Browse"}
                      </span>
                    </div>
                  </div>
                </div>
