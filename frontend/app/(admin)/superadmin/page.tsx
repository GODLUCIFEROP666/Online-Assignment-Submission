  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        {/* ── Global Statistics — sm:2 → md:3 → xl:6 to avoid orphan cards ── */}
        ...
        <section className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm space-y-5">
          ...
          {/* Teachers table — overflow-x-auto for mobile horizontal scroll */}
                  ...
                  teachers.map((teacher) => (
                    <tr key={teacher.id} className="hover:bg-slate-50/50">
                      <td className="px-4 py-3">
                        <div className="font-bold text-slate-800 whitespace-nowrap">{teacher.name}</div>
                        <div className="text-xs text-slate-400">@{teacher.username}</div>
                      </td>
                      <td className="px-4 py-3 text-slate-600">
                        <span className="break-anywhere">{teacher.email}</span>
                      </td>
                      <td className="px-4 py-3 text-slate-600 whitespace-nowrap">{teacher.college ?? "-"}</td>
                      <td className="px-4 py-3 text-slate-600 whitespace-nowrap">{teacher.course ?? "-"}</td>
                      <td className="px-4 py-3">
                        <div className="flex justify-end gap-1.5">
                          <button
                            type="button"
                            onClick={() => beginTeacherEdit(teacher)}
                            className="inline-flex items-center gap-1 rounded-xl border border-slate-200 bg-white px-2.5 py-2 text-xs font-bold text-slate-600 hover:border-indigo-200 hover:text-indigo-600"
                          >
                          ...
                          <button
                            ...
                            className="inline-flex items-center gap-1 rounded-xl border border-amber-200 bg-amber-50 px-2.5 py-2 text-xs font-bold text-amber-700 hover:bg-amber-100"
                          >
        ...
        <section className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm space-y-5">
          ...
          {/* College form — stacks on mobile, side-by-side on lg+ */}
          <div className="grid gap-4 grid-cols-1 lg:grid-cols-[1fr_auto]">
            ...
            <div className="flex flex-wrap items-end gap-3">
              <button
                ...
                className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 rounded-2xl bg-slate-900 px-5 py-3 text-xs font-bold text-white shadow-sm hover:bg-slate-800 disabled:opacity-60"
              >
              ...
              {editingCollegeId ? (
                <button
                  ...
                  className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-5 py-3 text-xs font-bold text-slate-600 hover:bg-slate-50"
                >
                  <X className="h-4 w-4" />
                  <span>Cancel Edit</span>
                </button>
          ...
          {/* Colleges table — overflow-x-auto for mobile scroll */}
          <div className="overflow-x-auto rounded-2xl border border-slate-100">
            <table className="min-w-full divide-y divide-slate-100 text-left text-sm">
              <thead className="bg-slate-50 text-xs font-bold uppercase tracking-wider text-slate-500">
                <tr>
        ...
        <section className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm space-y-5">
          ...
          {/* Students table — overflow-x-auto for mobile scroll */}
          <div className="overflow-x-auto rounded-2xl border border-slate-100">
            <table className="min-w-full divide-y divide-slate-100 text-left text-sm">
              <thead className="bg-slate-50 text-xs font-bold uppercase tracking-wider text-slate-500">
                <tr>
                ...
                {students.length === 0 ? (
                  ...
                  students.map((student) => (
                    <tr key={student.username} className="hover:bg-slate-50/50">
                      ...
                      <td className="px-4 py-3 text-slate-600 whitespace-nowrap">{student.course_year ?? "-"}</td>
